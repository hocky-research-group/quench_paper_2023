...
for run_temp in run_temp_list:
    ...
    # generate rho_ij_kl
    rho_file = os.path.join(out_dir,"ref_rho_%dx%d_%dx%d_k%.1f_tt%.1f.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp))
    if not os.path.exists(rho_file):
        ...
        for i,phi in enumerate(phi_centers):
            for j,psi in enumerate(psi_centers):
                ...
                rho_ij = np.load(os.path.join(in_dir,"rho_list_umbrella_phi%.2f_psi%.2f_k%.1f_tt%.1f_%dx%d_Nstep1000.npy"%(phi,psi,kappa,target_temp,fes_phi_windows,fes_psi_windows)))
                rho_ij_kl[i,j,:,:] = np.mean(rho_ij,axis=0)
        np.save(os.path.join(out_dir,"ref_rho_%dx%d_%dx%d_k%.1f_tt%.1f.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp)),rho_ij_kl)
        print("shape",rho_ij_kl.shape)
    
    # initialize
    rho_ij_kl = np.load(os.path.join(out_dir,"ref_rho_%dx%d_%dx%d_k%.1f_tt%.1f.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp)))
