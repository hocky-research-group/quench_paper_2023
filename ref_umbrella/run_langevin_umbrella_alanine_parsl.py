...
for run_temp in run_temp_list:
    ...
    for phi in phi_centers:
        for psi in psi_centers:
            ...
            #pull_plumed_file = os.path.join(out_umbrella_dir,"pull_" + os.path.basename(plumed_template))
            #if not os.path.exists(pull_plumed_file):
            #    pull_plumed_file = make_plumed_file(plumed_template,out_umbrella_dir,phi,psi,pull_kappa,pull_kappa,out_prefix="pull_")
            pull_plumed_file = None
            #r = run_langevin_umbrella_alanine(command_file,input_file,out_prefix,run_gamma,run_temp,eq_steps,thermo_freq,run_steps,nrestart=nrestart,dt=dt,plumed_file=plumed_file,pull_plumed_file=pull_plumed_file)
