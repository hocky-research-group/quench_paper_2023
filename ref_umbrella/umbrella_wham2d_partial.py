...
for run_temp in run_temp_list:
    ...
    for target_temp in target_temp_list:
        ...
        N_list = [1000,2000,5000,10000,20000,50000,100000,200000,500000,600000,700000,800000,900000,910000,920000,930000,940000,950000,960000,970000,980000,990000,1000000,1500000,2000000]
        for N in N_list:
            F_file = os.path.join(out_dir,"ref_F_%dx%d_k%.1f_tt%.1f_N%d.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,N))
            if os.path.exists(F_file):
                continue
            # generate rho_ij_kl
            rho_file = os.path.join(out_dir,"ref_rho_%dx%d_%dx%d_k%.1f_tt%.1f_N%d.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,N))
            if not os.path.exists(rho_file):
                ...
                for i,phi in enumerate(phi_centers):
                    for j,psi in enumerate(psi_centers):
                        ...
                        if N < 1000:
                            rho_ij = np.load(os.path.join(in_dir,"rho_list_umbrella_phi%.2f_psi%.2f_k%.1f_tt%.1f_%dx%d_N2000.npy"%(phi,psi,kappa,target_temp,fes_phi_windows,fes_psi_windows)))[:N,:,:]
                            rho_ij_kl[i,j,:,:] = np.mean(rho_ij,axis=0)
                        else:
                            rho_ij = np.load(os.path.join(in_dir,"rho_list_umbrella_phi%.2f_psi%.2f_k%.1f_tt%.1f_%dx%d_Nstep1000.npy"%(phi,psi,kappa,target_temp,fes_phi_windows,fes_psi_windows)))[:int(N/1000),:,:]
                            rho_ij_kl[i,j,:,:] = np.mean(rho_ij,axis=0)
                np.save(os.path.join(out_dir,"ref_rho_%dx%d_%dx%d_k%.1f_tt%.1f_N%d.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,N)),rho_ij_kl)
                print("shape",rho_ij_kl.shape)
            
            # initialize
            rho_ij_kl = np.load(os.path.join(out_dir,"ref_rho_%dx%d_%dx%d_k%.1f_tt%.1f_N%d.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,N)))
            ...
            # save results
            np.save(os.path.join(out_dir,"ref_rho_%dx%d_k%.1f_tt%.1f_N%d.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,N)),rho_kl)
            np.save(os.path.join(out_dir,"ref_F_%dx%d_k%.1f_tt%.1f_N%d.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,N)),F_kl)
            rho_kl = np.load(os.path.join(out_dir,"ref_rho_%dx%d_k%.1f_tt%.1f_N%d.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,N)))
            F_kl = np.load(os.path.join(out_dir,"ref_F_%dx%d_k%.1f_tt%.1f_N%d.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,N)))
            print(F_kl.shape)
