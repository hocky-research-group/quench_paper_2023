...
gT = gT0 + dgT
gT_b = gT0_b - dgT
in_dir_prefix = os.path.join(os.path.dirname(os.getcwd()),"ref_solv_umbrella","high_T_umbrella","T%.1f"%(run_temp))
...
for phi in phi_centers:
    ...
    for psi in psi_centers:
        ...
        print("phi%.2f,psi%.2f,kappa%.1f,%d restart files"%(phi,psi,kappa,len(restart_files)))
        # make plumed.dat
        ...
        if not os.path.exists(plumed_file):
            plumed_file = make_plumed_file(plumed_template,out_dir,phi,psi,kappa,kappa)
        for quench_gamma in quench_gamma_list[:1]:
            for restart_file in restart_files:
                ...
                if not os.path.exists(final_restart_file_b):
                    r = run_quench(command_file,restart_file,out_dir,quench_thermo_freq,-quench_gamma,gT,gT_b,dt=dt,heat=True,plumed_file=plumed_file) # change restart_b if changing dt value!!
                    result_list.append(r)
