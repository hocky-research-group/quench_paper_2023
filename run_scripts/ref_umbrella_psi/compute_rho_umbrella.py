...
for run_temp in run_temp_list:
    ...
    for target_temp in target_temp_list:
        ...
        for psi in psi_centers:
            ...
            print("psi%.1f,rt%.1f,tt%.1f"%(psi,run_temp,target_temp))
            final_file = os.path.join(out_dir,"rho_list_umbrella_psi%.2f_k%.1f_tt%.1f_%dx%d_Nstep1000.npy"%(psi,kappa,target_temp,fes_phi_windows,fes_psi_windows))
            ...
            for restart_file in restart_files: # only one log file
                ...
                np.save(os.path.join(out_dir,"rho_list_umbrella_psi%.2f_k%.1f_tt%.1f_%dx%d_N2000.npy"%(psi,kappa,target_temp,fes_phi_windows,fes_psi_windows)),rho_list[:2000,:,:])
                rho = np.mean((rho_list).reshape(1000,2000,fes_phi_windows,fes_psi_windows),axis=0)
                np.save(os.path.join(out_dir,"rho_list_umbrella_psi%.2f_k%.1f_tt%.1f_%dx%d_Nstep1000.npy"%(psi,kappa,target_temp,fes_phi_windows,fes_psi_windows)),rho)
