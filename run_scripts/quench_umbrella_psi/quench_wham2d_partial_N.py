...
for target_temp in target_temp_list:
    ...
    for quench_gamma in quench_gamma_list:
        for num_restart in num_restart_list:
            # generate rho_ij_kl
            rho_file = os.path.join(out_dir,"N_infinite_stopping_%d_%dx%d_k%.1f_tt%.1f_qg%.2e_N%d.npy"%(psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma,num_restart))
            if not os.path.exists(rho_file):
                N_j_kl = np.zeros((psi_windows,fes_phi_windows,fes_psi_windows))
                for j,psi in enumerate(psi_centers):
                    in_dir = os.path.join(in_dir_prefix,"psi%.2f_k%.1f"%(psi,kappa))
                    N_j_list = np.load(os.path.join(in_dir,"infinite_stopping_N_list_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)))
                    N_j_list = N_j_list[:num_restart,:,:]
                    N_j = np.zeros((fes_phi_windows,fes_psi_windows))
                    for k in range(fes_phi_windows):
                        for l in range(fes_psi_windows):
                            N_j[k,l] = np.sum(N_j_list[:,k,l])
                    # normalize
                    N_j_kl[j,:,:] = N_j
                np.save(os.path.join(out_dir,"N_infinite_stopping_%d_%dx%d_k%.1f_tt%.1f_qg%.2e_N%d.npy"%(psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma,num_restart)),N_j_kl)
                print("shape",N_j_kl.shape)
            
            # initialize
            rho_ij_kl = np.load(os.path.join(out_dir,"N_infinite_stopping_%d_%dx%d_k%.1f_tt%.1f_qg%.2e_N%d.npy"%(psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma,num_restart)))
            ...
            # save results
            np.save(os.path.join(out_dir,"infinite_stopping_psi_rho_%dx%d_k%.1f_rt%.1f_tt%.1f_qg%.2e_N%d_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,run_temp,target_temp,quench_gamma,num_restart)),rho_kl)
            np.save(os.path.join(out_dir,"infinite_stopping_psi_F_%dx%d_k%.1f_rt%.1f_tt%.1f_qg%.2e_N%d_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,run_temp,target_temp,quench_gamma,num_restart)),F_kl)
            rho_kl = np.load(os.path.join(out_dir,"infinite_stopping_psi_rho_%dx%d_k%.1f_rt%.1f_tt%.1f_qg%.2e_N%d_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,run_temp,target_temp,quench_gamma,num_restart)))
            F_kl = np.load(os.path.join(out_dir,"infinite_stopping_psi_F_%dx%d_k%.1f_rt%.1f_tt%.1f_qg%.2e_N%d_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,run_temp,target_temp,quench_gamma,num_restart)))
            print(F_kl)
