...
for target_temp in target_temp_list:
    ...
    for quench_gamma in quench_gamma_list: 
        # generate rho_ij_kl
        rho_file = os.path.join(out_dir,"infinite_stopping_rho_%d_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma))
        ...
        if True:
            ...
            for j,psi in enumerate(psi_centers):
                ...
                # infinite_stopping_lnrho_psi2.98_k24.0_qg1.00e-03_tt300.0_100x100.npy
                ...
                lnrho_ij = np.ones((fes_phi_windows,fes_psi_windows))
                ...
                lnQ_ij = log_sum(lnQ_ij_list)
                rho_ij_kl[j,:,:] = np.exp(lnrho_ij-lnQ_ij)
            print(rho_ij_kl)
            np.save(os.path.join(out_dir,"infinite_stopping_rho_%d_%dx%d_k%.1f_qg%.2e_tt%.1f.npy"%(psi_windows,fes_phi_windows,fes_psi_windows,kappa,quench_gamma,target_temp)),rho_ij_kl)
            print("shape",rho_ij_kl.shape)
        
        # initialize
        rho_ij_kl = np.load(os.path.join(out_dir,"infinite_stopping_rho_%d_%dx%d_k%.1f_qg%.2e_tt%.1f.npy"%(psi_windows,fes_phi_windows,fes_psi_windows,kappa,quench_gamma,target_temp)))
