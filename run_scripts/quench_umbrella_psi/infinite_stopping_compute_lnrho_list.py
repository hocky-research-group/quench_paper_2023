        ...
        for psi in psi_centers:
            ...
            for r in result_list:
                try:
                    ...
                    print(f"Application {r} failed but continuing.")
            # save data
            np.save(os.path.join(out_dir,"infinite_stopping_lnrho_list_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)),lnrho_list)
            ...
            np.save(os.path.join(out_dir,"infinite_stopping_lnrho_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)),lnrho)
            np.save(os.path.join(out_dir,"infinite_stopping_lnQ_list_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)),lnQ_list)
            np.save(os.path.join(out_dir,"infinite_stopping_lnQ_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)),log_sum(lnQ_list))
            np.save(os.path.join(out_dir,"infinite_stopping_time_list_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)),time_list)
