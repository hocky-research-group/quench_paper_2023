...
for run_temp in run_temp_list:
    ...
    for quench_gamma in quench_gamma_list: 
        # generate rho_ij_kl
        rho_file = os.path.join(out_dir,"N_%dx%d_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma))
        if not os.path.exists(rho_file):
            ...
            for i,phi in enumerate(phi_centers):
                for j,psi in enumerate(psi_centers):
                    ...
                    N_ij = np.load(os.path.join(in_dir,"N_phi%.2f_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(phi,psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)))
                    rho_ij_kl[i,j,:,:] = N_ij
            np.save(os.path.join(out_dir,"rho_%dx%d_%dx%d_k%.1f_qg%.2e_tt%.1f.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,quench_gamma,target_temp)),rho_ij_kl)
            print("shape",rho_ij_kl.shape)
        
        # initialize
        rho_ij_kl = np.load(os.path.join(out_dir,"rho_%dx%d_%dx%d_k%.1f_qg%.2e_tt%.1f.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,quench_gamma,target_temp)))
        ...
        # save results
        np.save(os.path.join(out_dir,"rho_%dx%d_k%.1f_tt%.1f_qg%.2e_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)),rho_kl)
        np.save(os.path.join(out_dir,"F_%dx%d_k%.1f_tt%.1f_qg%.2e_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)),F_kl)
        rho_kl = np.load(os.path.join(out_dir,"rho_%dx%d_k%.1f_tt%.1f_qg%.2e_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)))
        F_kl = np.load(os.path.join(out_dir,"F_%dx%d_k%.1f_tt%.1f_qg%.2e_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)))
        print(F_kl)
        ...
        ax = plt.gca()
        im = ax.imshow(F_kl.T,origin='lower',extent=(-np.pi,np.pi,-np.pi,np.pi),vmin=0.0,vmax=20.0)
        plt.xlabel("$\\phi$")
        plt.ylabel("$\\psi$")
        plt.title("$T_0=%.1f$K,quench+umbrella"%(run_temp))
        divider = make_axes_locatable(ax)
        cax = divider.append_axes("right", size="5%", pad=0.05)
        plt.colorbar(im,cax=cax,label="Free Energy (kcal/mol)")
        plt.tight_layout()
        #plt.show()
        plt.savefig("FES_quench_umbrella_%dx%d_k%.1f_rt%.1f_qg%.2e_N.png"%(fes_phi_windows,fes_psi_windows,kappa,run_temp,quench_gamma),dpi=400)
        plt.close()
