...
for quench_gamma in quench_gamma_list: 
    # generate rho_ij_kl
    rho_file = os.path.join(out_dir,"rho_%dx%d_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma))
    if not os.path.exists(rho_file):
        ...
        for i,phi in enumerate(phi_centers):
            for j,psi in enumerate(psi_centers):
                ...
                lnrho_ij = np.load(os.path.join(in_dir,"lnrho_phi%.2f_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(phi,psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)))
    ...
    # save results
    np.save(os.path.join(out_dir,"rho_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)),rho_kl)
    np.save(os.path.join(out_dir,"F_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)),F_kl)
    rho_kl = np.load(os.path.join(out_dir,"rho_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)))
    F_kl = np.load(os.path.join(out_dir,"F_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)))
    print(F_kl)
