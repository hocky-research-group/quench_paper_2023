from quench_library import infinite_stopping_compute_N_2d,find_Ebound
import numpy as np
import os,sys
import glob
            ...
            for psi in psi_centers:
                ...
                for r in result_list:
                    try:
                        ...
                        print(f"Application {r} failed but continuing.")
                # save data
                np.save(os.path.join(out_dir,"infinite_stopping_N_list_phi%.2f_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(phi,psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)),N_list)
                np.save(os.path.join(out_dir,"infinite_stopping_N_phi%.2f_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(phi,psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)),np.sum(N_list,axis=0))
