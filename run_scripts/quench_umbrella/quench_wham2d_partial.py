...
for target_temp in target_temp_list:
    ...
    for num_restart in num_restart_list:
        for quench_gamma in quench_gamma_list: 
            # generate rho_ij_kl
            rho_file = os.path.join(out_dir,"infinite_stopping_rho_%dx%d_%dx%d_k%.1f_tt%.1f_qg%.2e_N%d.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma,num_restart))
            #if not os.path.exists(rho_file):
            if True:
                rho_ij_kl = np.zeros((phi_windows,psi_windows,fes_phi_windows,fes_psi_windows))
                for i,phi in enumerate(phi_centers):
                    for j,psi in enumerate(psi_centers):
                        in_dir = os.path.join(in_dir_prefix,"phi%.2f_psi%.2f_k%.1f"%(phi,psi,kappa))
                        lnrho_list_ij = np.load(os.path.join(in_dir,"infinite_stopping_lnrho_list_phi%.2f_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(phi,psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)))[:num_restart,:,:]
                        lnrho_ij = np.ones((fes_phi_windows,fes_psi_windows)) * -np.inf
                        for k in range(fes_phi_windows):
                            for l in range(fes_psi_windows):
                                lnrho_ij[k,l] = log_sum(lnrho_list_ij[:,k,l])
                        lnQ_list_ij = np.load(os.path.join(in_dir,"infinite_stopping_lnQ_list_phi%.2f_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(phi,psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)))[:num_restart]
                        lnQ_ij = log_sum(lnQ_list_ij)
                        lnrho_ij -= lnQ_ij
                        rho_ij_kl[i,j,:,:] = np.exp(lnrho_ij)
                np.save(os.path.join(out_dir,"infinite_stopping_rho_%dx%d_%dx%d_k%.1f_qg%.2e_tt%.1f_N%d.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,quench_gamma,target_temp,num_restart)),rho_ij_kl)
                print("shape",rho_ij_kl.shape)
            
            # initialize
            rho_ij_kl = np.load(os.path.join(out_dir,"infinite_stopping_rho_%dx%d_%dx%d_k%.1f_qg%.2e_tt%.1f_N%d.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,quench_gamma,target_temp,num_restart)))
            ...
            while(error > tolerance):
                ...
                for k in range(fes_phi_windows):
                    for l in range(fes_psi_windows):
                        #rho_kl[k,l] = N_kl[k,l] / np.sum(N_ij * np.exp(-w_ij_kl[:,:,k,l]/run_kbt) / old_c_ij_inverse)
                ...
                for i in range(phi_windows):
                    for j in range(psi_windows):
                        #c_ij_inverse[i,j] = np.sum(rho_kl * np.exp(-w_ij_kl[i,j,:,:]/run_kbt))
                        c_ij_inverse[i,j] = np.sum(rho_kl * np.exp(-w_ij_kl[i,j,:,:]/target_kbt))
            ...
            # save results
            np.save(os.path.join(out_dir,"infinite_stopping_rho_%dx%d_k%.1f_tt%.1f_qg%.2e_N%d.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma,num_restart)),rho_kl)
            np.save(os.path.join(out_dir,"infinite_stopping_F_%dx%d_k%.1f_tt%.1f_qg%.2e_N%d.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma,num_restart)),F_kl)
            rho_kl = np.load(os.path.join(out_dir,"infinite_stopping_rho_%dx%d_k%.1f_tt%.1f_qg%.2e_N%d.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma,num_restart)))
            F_kl = np.load(os.path.join(out_dir,"infinite_stopping_F_%dx%d_k%.1f_tt%.1f_qg%.2e_N%d.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma,num_restart)))
            print(F_kl)
