...
for target_temp in target_temp_list:
    ...
    for quench_gamma in quench_gamma_list:
        for num_restart in num_restart_list:
            # generate rho_ij_kl
            rho_file = os.path.join(out_dir,"infinite_stopping_N_%dx%d_%dx%d_k%.1f_tt%.1f_qg%.2e_N%d.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma,num_restart))
            if not os.path.exists(rho_file):
                ...
                for i,phi in enumerate(phi_centers):
                    for j,psi in enumerate(psi_centers):
                        ...
                        N_ij_list = np.load(os.path.join(in_dir,"infinite_stopping_N_list_phi%.2f_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(phi,psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)))
                        N_ij_list = N_ij_list[:num_restart,:,:]
                        N_ij = np.sum(N_ij_list,axis=0)
                        # normalize
                        N_ij_kl[i,j,:,:] = N_ij
                np.save(os.path.join(out_dir,"infinite_stopping_N_%dx%d_%dx%d_k%.1f_tt%.1f_qg%.2e_N%d.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma,num_restart)),N_ij_kl)
                print("shape",N_ij_kl.shape)
            
            # initialize
            rho_ij_kl = np.load(os.path.join(out_dir,"infinite_stopping_N_%dx%d_%dx%d_k%.1f_tt%.1f_qg%.2e_N%d.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma,num_restart)))
            ...
            # save results
            np.save(os.path.join(out_dir,"infinite_stopping_rho_%dx%d_k%.1f_rt%.1f_tt%.1f_qg%.2e_N%d_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,run_temp,target_temp,quench_gamma,num_restart)),rho_kl)
            np.save(os.path.join(out_dir,"infinite_stopping_F_%dx%d_k%.1f_rt%.1f_tt%.1f_qg%.2e_N%d_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,run_temp,target_temp,quench_gamma,num_restart)),F_kl)
            #rho_kl = np.load(os.path.join(out_dir,"rho_%dx%d_k%.1f_rt%.1f_tt%.1f_qg%.2e_N%d_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,run_temp,target_temp,quench_gamma,num_restart)))
            #F_kl = np.load(os.path.join(out_dir,"F_%dx%d_k%.1f_rt%.1f_tt%.1f_qg%.2e_N%d_N.npy"%(fes_phi_windows,fes_psi_windows,kappa,run_temp,target_temp,quench_gamma,num_restart)))
            print(F_kl)
