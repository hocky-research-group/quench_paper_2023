...
for target_temp in target_temp_list:
    ...
    for quench_gamma in quench_gamma_list: 
        # generate rho_ij_kl
        rho_file = os.path.join(out_dir,"infinite_stopping_rho_%dx%d_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma))
        if not os.path.exists(rho_file):
            ...
            for i,phi in enumerate(phi_centers):
                for j,psi in enumerate(psi_centers):
                    ...
                    lnrho_list_ij = np.load(os.path.join(in_dir,"infinite_stopping_lnrho_list_phi%.2f_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(phi,psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)))
                    ...
                    lnQ_list_ij = np.load(os.path.join(in_dir,"infinite_stopping_lnQ_list_phi%.2f_psi%.2f_k%.1f_qg%.2e_tt%.1f_%dx%d.npy"%(phi,psi,kappa,quench_gamma,target_temp,fes_phi_windows,fes_psi_windows)))
                    lnQ_ij = log_sum(lnQ_list_ij)
                    lnrho_ij -= lnQ_ij
                    rho_ij_kl[i,j,:,:] = np.exp(lnrho_ij)
            np.save(os.path.join(out_dir,"infinite_stopping_rho_%dx%d_%dx%d_k%.1f_qg%.2e_tt%.1f.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,quench_gamma,target_temp)),rho_ij_kl)
            print("shape",rho_ij_kl.shape)
        
        # initialize
        rho_ij_kl = np.load(os.path.join(out_dir,"infinite_stopping_rho_%dx%d_%dx%d_k%.1f_qg%.2e_tt%.1f.npy"%(phi_windows,psi_windows,fes_phi_windows,fes_psi_windows,kappa,quench_gamma,target_temp)))
        ...
        # save results
        np.save(os.path.join(out_dir,"infinite_stopping_rho_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)),rho_kl)
        np.save(os.path.join(out_dir,"infinite_stopping_F_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)),F_kl)
        rho_kl = np.load(os.path.join(out_dir,"infinite_stopping_rho_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)))
        F_kl = np.load(os.path.join(out_dir,"infinite_stopping_F_%dx%d_k%.1f_tt%.1f_qg%.2e.npy"%(fes_phi_windows,fes_psi_windows,kappa,target_temp,quench_gamma)))
        print(F_kl)
